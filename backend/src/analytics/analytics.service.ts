...
export class AnalyticsService {
  ...
  // ─── Revenue (POS + Online merged by day) ─────────────────────────────────

  async getRevenue(storeId: string, from: string, to: string) {
    ...
    for (const row of onlineOrders) {
      const d = String(row.date).substring(0, 10);
      if (!dateMap[d]) dateMap[d] = { date: d, pos_revenue: 0, pos_transactions: 0, pos_gst: 0, online_revenue: 0, online_orders: 0, online_gst: 0 };
  ...
  async getTopProducts(storeId: string, limit: number, periodDays: number) {
  ...
  async getSupplierScorecard(storeId: string) {
  ...
  async getHourlyHeatmap(storeId: string, periodDays: number) {
    const since = new Date();
    since.setDate(since.getDate() - periodDays);
  ...
  async getCategoryMix(storeId: string, periodDays: number) {
    const since = new Date();
    since.setDate(since.getDate() - periodDays);
  ...
  // ─── Network Summary (multi-store / chain rollup) ─────────────────────────

  async getNetworkSummary(from: string, to: string) {
    const fromDate = new Date(from);
    const toDate   = new Date(to);

    const storeStats = await this.prisma.$queryRaw<any[]>`
      SELECT
        s.id   AS store_id,
